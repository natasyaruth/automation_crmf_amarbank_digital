...
/*
 * This part I want to set RC 200 Cek data verihub
 * */
...
if (checkConditionMockServer == true) {
	...
	if (WebUI.waitForElementVisible(fieldInputKeyword, 5)) {
		...
		WebUI.setText(fieldInputKeyword, '[STAGING] CRMF Biometric Verificator Facematch')
		
		WebUI.sendKeys(fieldInputKeyword, Keys.chord(Keys.ENTER))
		
		TestObject checkConfigAvailable = new TestObject().addProperty('text',ConditionType.CONTAINS,'[STAGING] CRMF Biometric Verificator Facematch')	
		...
		if (isAvailable == true) {
			...
			if (isAvailaibleDrpDwn == true) {
				...
				String body = '{"code":1102,"fieldErrors":[{"target":"identity","messageKey":"validation.is.digit","message":"must contain digits only"}],"message":"invalid parameters","messageKey":"err.invalid.parameter"}'
			...
			keylogger.markFailed('Failed to Update Check Data Verihub V2')
