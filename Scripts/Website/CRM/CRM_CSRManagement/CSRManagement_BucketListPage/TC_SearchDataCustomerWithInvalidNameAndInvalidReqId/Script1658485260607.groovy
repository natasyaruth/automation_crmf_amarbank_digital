...
/*'We want to check the invalid request ID and Name Customer with Failed condition'*/
...
if (checkHeaderCsrManagement == true) {
	for (int i=0;i<invalidReqId.size();i++) {
		...
		WebUI.verifyOptionsPresent(drpDwnChooseStatusCard, ["Semua","Belum Aktivasi","Sudah Aktivasi","Block Kartu ATM","Permintaan Kartu Baru"])
			...
			TestObject txtHeaderCsrManagement = new TestObject().addProperty('text', ConditionType.CONTAINS , headerCSRManagementText)
			WebUI.verifyElementPresent(txtHeaderCsrManagement, 5)
			boolean txtRekening = WebUI.verifyElementVisible(txtHeaderCsrManagement)
			if (txtRekening == true) {
					boolean existFieldReqId = WebUI.verifyElementVisible(fieldReqId)
					if (existFieldReqId == true) {
						WebUI.setText(fieldReqId, invalidReqId.get(i))
						WebUI.setText(fieldNameCust, invalidNameCust.get(i))
