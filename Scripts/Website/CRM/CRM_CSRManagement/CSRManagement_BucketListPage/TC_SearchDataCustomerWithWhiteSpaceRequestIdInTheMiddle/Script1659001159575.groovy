...
/*'We want to check the white space in the middle input request ID'*/
		...
		WebUI.selectOptionByLabel(drpDwnChooseStatusCard, "Semua", false)	
				...
				if (inBucketListPage == true) {
					...
					if (existFieldReqId == true) {
						...
						TestObject textInvalidReqId = new TestObject().addProperty('text', ConditionType.CONTAINS , "Request ID wajib angka saja")
						boolean invalidInputReqId = WebUI.verifyElementPresent(textInvalidReqId, 5)
						if (invalidInputReqId == true) {
