 ...
 * Precondition:
	- Already on pop up sequrity question "Nama Ibu Kandung"
 * 
 * And have steps:
	- Input the true mother's name
	- Click on "Lanjutkan"
 * 
					...
					if (WebUI.waitForElementPresent(csrSecurityQuestion, 5)) {
						...
						if (WebUI.waitForElementPresent(fieldInputEmailSecQuest, 5)) {
							...
							if (WebUI.verifyElementPresent(csrEmailQuest, 5,FailureHandling.OPTIONAL)) {
								WebUI.setText(fieldInputEmailSecQuest, "dammofisona-1758@yopmail.com")
								WebUI.click(btnSubmitSecQuest)
							...
							while (loopPageChangeLog == false) {
								...
								for (int j = 0;j < listRowsTblChgLog.size(); j++) {
									...
									if (listColsTblChgLog.get(6).getText().equalsIgnoreCase('Succeed')) {
										...
										if (WebUI.waitForElementPresent(logoSenyumku, 5)) {
											WebUI.click(logoLoginPage)
											WebUI.waitForElementPresent(menuDashboard, 5)
											WebUI.click(menuCsrManagement)
											if (WebUI.waitForElementPresent(notifBlock, 5)) {
												WebUI.click(btnAbort)
												WebUI.waitForElementPresent(txtAccountNumb, 5)
												WebUI.setText(txtAccountNumb, accountNumb)
												WebUI.click(btnSubmitSearch)
												WebUI.click(btnDetail1)
												TestObject csrDetailPageAfterLogout = new TestObject().addProperty('text',ConditionType.CONTAINS,'Customer Detail')
												if (WebUI.verifyElementPresent(csrDetailPageAfterLogout, 5)) {
													WebUI.scrollToElement(linkDataChangeLog, 5)
													WebUI.click(linkDataChangeLog)
													TestObject statementViewd = new TestObject().addProperty('text',ConditionType.CONTAINS,'Viewed')
													if (WebUI.verifyElementPresent(statementViewd, 5)) {
														keylogger.markPassed('We already catch the case')
														WebUI.click(btnBackDashboard)
														WebUI.waitForPageLoad(5)
														break loopPage
														break loopChangeLog
													} else {keylogger.markError('Status viewed not found')}
												} else {keylogger.markError('We are not in customer detail')}
											} else {keylogger.markError('Process holding customer number is failed')}
										} else {keylogger.logInfo('We not logout')}
									} else {
