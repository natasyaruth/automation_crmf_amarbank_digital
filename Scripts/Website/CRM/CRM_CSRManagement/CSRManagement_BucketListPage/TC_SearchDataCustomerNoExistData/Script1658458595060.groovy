...
/* We want to check the conditional if we search by "Invalid or not register Request ID" and we catch the error or execption message*/
...
if (checkDropDownStatus == true) {
	for (int i=0;i<checkListByOrder.size();i++) {
		...
		boolean fieldRequestId = WebUI.verifyElementVisible(fieldReqId)
		if (fieldRequestId == true) {
			keyLogger.markPassed("We can do it for search the invalid data")
			WebUI.setText(fieldReqId, checkListByOrder.get(i))
			...
			keyLogger.markFailed("We not find the Field Request Id")
		}
	}
} else {
