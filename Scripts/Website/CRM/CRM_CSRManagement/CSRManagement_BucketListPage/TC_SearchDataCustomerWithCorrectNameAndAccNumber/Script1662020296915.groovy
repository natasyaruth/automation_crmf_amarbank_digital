...
if(isObjectNotFound) {
	...
	keyLogger.markFailed("Customer with data "+custName+" and "+accNumber+" is not found")
	...
	if(actCustName.equals(custName)) {
		...
		if(actAccNumber.equals(accNumber)) {
			...
			WebUI.takeScreenshot()
			keyLogger.markPassed("Customer with data Fullname: "+custName+" and Account number: "+accNumber+" is found")
			
		} else {
			
			/* Take screenshot, mark as failed case and print info message error*/
			WebUI.takeScreenshot()
			keyLogger.markFailed("Account number customer with  fullname "+custName+" is not same")
