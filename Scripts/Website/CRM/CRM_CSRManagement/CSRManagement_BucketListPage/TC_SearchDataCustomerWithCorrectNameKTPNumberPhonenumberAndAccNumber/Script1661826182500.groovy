...
if(isObjectNotFound) {
	...
	keyLogger.markFailed("Customer with data "+custName+", "+ktpNumber+", "+phoneNumber+" and"+accNumber+" is not found")
	...
	if(actCustName.equals(custName)) {
		...
		if(actPhoneNumber.equals(phoneNumber)) {
			...
			if(actAccNumber.equals(accNumber)) {
				...
				WebUI.takeScreenshot()
				keyLogger.markPassed("Customer with data "+custName+", "+ktpNumber+", "+phoneNumber+" and"+accNumber+" is found")
				
			} else {
				
				/* Take screenshot, mark as failed case and print info message error*/
				WebUI.takeScreenshot()
				keyLogger.markFailed("Account number customer with  fullname is "+custName+", KTP Number "+ktpNumber+", and Phonenumber "+phoneNumber+" is not same")
