...
if(isObjectNotFound) {
	...
	keyLogger.markFailed("Customer with data "+custName+", "+phoneNumber+" and "+accNumber+" is not found")
	...
	if(actCustName.equals(custName)) {
		...
		if(actPhoneNumber.equals(phoneNumber)) {
			...
			if(actAccNumber.equals(accNumber)) {
				...
				WebUI.takeScreenshot()
				keyLogger.markPassed("Customer with data Fullname: "+custName+", Phonenumber: "+phoneNumber+" and Account number: "+accNumber+" is found")
			
			} else {
				
				/* Take screenshot, mark as failed case and print info message error*/
				WebUI.takeScreenshot()
				keyLogger.markFailed("Account number customer with  fullname is "+custName+" and Phonenumber "+phoneNumber+" is not same")
		...
		/* Take screenshot, mark as failed case and print info message error*/
		WebUI.takeScreenshot()
		keyLogger.markFailed("Phonenumber customer with  fullname is "+custName+" is not same")
		...
		/* Take screenshot, mark as failed case and print info message error*/
		WebUI.takeScreenshot()
		keyLogger.markFailed("Name customer is not same")
