	...
	for (int i=0;i<checkListByOrder.size();i++) {
		...
		if (WebUI.verifyElementVisible(headerCustDataElement,FailureHandling.OPTIONAL)) {
			...
			if (txtRekening == true) {
				...
				boolean hoverKtp = WebUI.verifyElementVisible(btnIdKtp)
				if (hoverKtp == true) {
					WebUI.click(btnIdKtp)
					ktpNumber = WebUI.getAttribute(fieldNoKtp, "value")
				} else {
				...
				if (inBucketListPage == true) {
					...
					if (existFieldReqId == true) {
						...
						WebUI.setText(fieldIdNumber, ktpIdText)
