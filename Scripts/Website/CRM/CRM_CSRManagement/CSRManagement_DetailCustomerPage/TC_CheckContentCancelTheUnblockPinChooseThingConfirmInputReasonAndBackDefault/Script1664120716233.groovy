 ...
 * 	1. Click button 'Unblock PIN Transaksi'
	...
	5. Delete reason of block
	6. Click Batal
	7. Click button 'Unblock PIN Transaksi'
		...
		if (WebUI.verifyElementClickable(btnUnblockPin,FailureHandling.OPTIONAL)) {
			...
			if (WebUI.verifyElementPresent(modalUnblockPin, 5,FailureHandling.OPTIONAL)) {
				...
				if (WebUI.verifyElementChecked(chkMotherName, 5,FailureHandling.OPTIONAL)) {
					WebUI.click(chkMotherName)
					keylogger.markPassed('element already set unchecked')
				} else {keylogger.markError('Element not checked')}
				if (WebUI.verifyElementChecked(chkAccountNumber, 5,FailureHandling.OPTIONAL)) {
					WebUI.click(chkAccountNumber)
					keylogger.markPassed('element already set unchecked')
				} else {keylogger.markError('Element not checked')}
				if (WebUI.verifyElementChecked(chkPhoneNumber, 5,FailureHandling.OPTIONAL)) {
					WebUI.click(chkPhoneNumber)
					keylogger.markPassed('element already set unchecked')
				} else {keylogger.markError('Element not checked')}
				WebUI.clearText(fieldInputReasonUnblockPin)
				keylogger.markPassed('All field already back to default')
				...
				keylogger.markPassed('Button Blue Unblock PIN Transaksi with default is Enable')
				if (WebUI.verifyElementClickable(btnCancelUnblockPin,FailureHandling.OPTIONAL)) {
					...
					if (WebUI.verifyElementClickable(btnUnblockPin,FailureHandling.OPTIONAL)) {
						...
						WebUI.verifyElementNotChecked(chkPhoneNumber, 5)
						WebUI.click(btnCancelUnblockPin)
