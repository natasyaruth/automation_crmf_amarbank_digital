    ...
    if (WebUI.waitForElementVisible(SectionChangelog, 5)) {
        ...
        if (changeLogActivity == true) {
            ...
            if (WebUI.waitForElementVisible(DrpFilterActions, 5, FailureHandling.OPTIONAL)) {
						WebUI.selectOptionByIndex(DrpFilterSource, 'KYC Video Request')
						WebUI.verifyElementText(DrpFilterSource, 'KYC Video Request')
					} else {keyLogger.logInfo("We Not find the filter 'KYC Video Request'")}
				...
				WebUI.verifyMatch(SourcesKYCVideo, ChangelogSources, false)
