		...
		if (addressSentCard == true) {
			...
			if (WebUI.waitForElementClickable(btnEditAddressDelivery, 5, FailureHandling.OPTIONAL)) {
				...
				if (inputRw == true) {
					...
					WebUI.setText(txtRw,RandomStringUtils.randomNumeric(4))
				} else (inputRw == false) { keyLogger.logInfo("We don't find the element Rw") }
				WebUI.waitForElementVisible(btnSave, 5)
				WebUI.takeScreenshot()
				WebUI.click(btnSave)
				if (WebUI.waitForElementVisible(txtAlertMoreInputRw, 5, FailureHandling.OPTIONAL)) {
					WebUI.verifyElementPresent(txtAlertMoreInputRw, 5)
					WebUI.verifyElementText(txtAlertMoreInputRw, "Pastikan panjang karakter antara 1 dan 3")
					keyLogger.logInfo("We capture the alert text for fill the 'RW'")
					WebUI.takeScreenshot()
				} else {keyLogger.logInfo("We don't find the alert in 'RW'")}
			} else {
				WebUI.takeScreenshot()
