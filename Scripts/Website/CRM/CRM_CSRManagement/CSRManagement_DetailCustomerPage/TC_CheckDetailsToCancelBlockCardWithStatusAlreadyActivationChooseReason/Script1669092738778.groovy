 ...
 * 	1. Click button 'Block Kartu'
	2. Checklist all things to confirm
	3. Choose one of reason block
	4. Click button 'Block Kartu ATM'
	...
	4. Modal will dissapear and detail Data Kartu ATM is:
		...
		Button change into 'Unblock Kartu'
		Status card change into 'Blokir Kartu ATM'
...
if (WebUI.waitForElementVisible(drpCustType, 5)) {
	...
	if (WebUI.verifyElementPresent(csrDetaiPage, 5)) {
		...
		if (WebUI.verifyElementClickable(btnBlockCard)) {
		...
		if (WebUI.verifyElementPresent(pageBlockCardInfo, 5, FailureHandling.OPTIONAL)) {
			...
			if (checkWrd.contentEquals('Unblock Kartu')) {
				keylogger.markPassed('Button change into Unblock Kartu')
		...
		if (WebUI.waitForElementVisible(btnUnblockCard, 5)) {
			...
			if (WebUI.verifyElementPresent(unblockAtmCard, 5)) {
				WebUI.click(chkMotherName)
				WebUI.click(chkAccNumber)
				WebUI.click(chkAskPhoneNumber)
				if (WebUI.verifyElementClickable(btnUnblockAtmCard)) {
