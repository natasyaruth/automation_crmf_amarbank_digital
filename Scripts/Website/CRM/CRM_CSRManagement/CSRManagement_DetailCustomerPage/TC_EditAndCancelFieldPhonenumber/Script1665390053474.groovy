...
for (int i = 0; i < customerType.size(); i++) {
    ...
    if (WebUI.waitForElementVisible(elementNoHandphone, 5)) {
        ...
        if (phoneNumber == true) {
            ...
            if (WebUI.waitForElementVisible(txtNoHandphone, 5, FailureHandling.OPTIONAL)) {
				
                if (WebUI.verifyElementClickable(btnEditPhoneNumber, FailureHandling.OPTIONAL)) {
                    WebUI.click(btnEditPhoneNumber)
					def LabelPhoneNumb=WebUI.getText(LabelPhoneNumber)
					oldData = WebUI.getAttribute(txtNoHandphone, 'value')
					if (WebUI.verifyElementVisible(txtNoHandphone,FailureHandling.OPTIONAL)) {
						WebUI.setText(txtNoHandphone, "+628" +RandomStringUtils.randomNumeric(10))
						newData = WebUI.getAttribute(txtNoHandphone, 'value')
					} else {keyLogger.logInfo("We Not update phone number")}
					
                    if (WebUI.verifyElementVisible(btnCancelPhoneNumber, FailureHandling.OPTIONAL)) {
                        WebUI.takeScreenshot()

                        WebUI.click(btnCancelPhoneNumber)
                ...
                WebUI.verifyNotMatch(ChangelogField, LabelPhoneNumb , false)
				...
				WebUI.verifyNotMatch(ChangelogAction, txtAction, false)
