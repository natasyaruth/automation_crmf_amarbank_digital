	...
	for(int i=0;i<listRows.size();i++) {
		...
		if(WebUI.waitForElementClickable(btnBack, 5)) {
			...
			if(actStatusCust.equals(onProcessStat) || actStatusCust.equals(completedStat)) {
				...
				/* Verify if the button send link upload photos is exists, if it's not exists will move to else statement */
				if(WebUI.waitForElementPresent(titlePhotoEKTP, 3)) {
					
					if(WebUI.verifyTextNotPresent(txtNoImagesKTP, false)){
						
						WebUI.click(imgKTP)
						
						if(WebUI.verifyElementAttributeValue(imgKTPContainer, 'style', 'transform: rotate(0deg);', 2)) {
							
							keyLogger.markPassed("eKTP images can be rotate. Case SUCCESS")
							
						} else {
							
							WebUI.takeScreenshot()
							keyLogger.markFailed("eKTP image can't be rotate. Case FAILED")							
