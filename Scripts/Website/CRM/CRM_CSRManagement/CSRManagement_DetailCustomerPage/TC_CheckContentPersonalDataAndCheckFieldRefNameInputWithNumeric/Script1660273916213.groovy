 ...
 * 5. We want to input with numeric reference name
			...
			if (WebUI.waitForElementVisible(DataCustomer, 5, FailureHandling.OPTIONAL)) {
				if (WebUI.verifyElementClickable(btnEditDatCustomer,FailureHandling.OPTIONAL)) {
					...
					if (WebUI.verifyElementVisible(txtRefNumber,FailureHandling.OPTIONAL)) {
						WebUI.setText(txtRefNumber, RandomStringUtils.randomNumeric(30))
					...
					if (WebUI.verifyElementVisible(btnSavePersonalData,FailureHandling.OPTIONAL)) {
					  ...
					  if (WebUI.verifyElementVisible(alertReferenceNameInputWithNumeric,FailureHandling.OPTIONAL)) {
						  WebUI.verifyElementText(alertReferenceNameInputWithNumeric, "Nama kerabat tidak sesuai format")
						  WebUI.takeScreenshot()
