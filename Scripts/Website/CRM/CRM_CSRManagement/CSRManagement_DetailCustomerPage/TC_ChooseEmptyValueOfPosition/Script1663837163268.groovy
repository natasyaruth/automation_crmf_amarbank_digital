		...
		if(WebUI.waitForElementClickable(btnBack, 5)) {
			...
			if(actStatusCust.equals(onProcessStat) || actStatusCust.equals(completedStat)) {
				...
				if(WebUI.waitForElementClickable(btnEditEmploymentData, 3)) {
					...
					/* Verify if the dropdown position is exists, if it's not exists will move to else statement */
					if(WebUI.waitForElementPresent(drpPosition, 1)) {
						
						/* Select empty value (at index 0) in dropdown position */
						WebUI.selectOptionByIndex(drpPosition, 0)
						
					} else {
						
						/* Select value in dropdown work type to make dropdown position is appear*/
						WebUI.selectOptionByIndex(drpWorkType, 1)
					...
					if(WebUI.waitForElementPresent(msgErrorPosition, 2)) {
						
						/* Storing text of not found to variable actMsgError*/
						actMsgError = WebUI.getText(msgErrorPosition)
