 ...
 * 	1. Click button 'Unblock Kartu'
	...
	4. Delete reason of unblock
	5. Uncheck things to confirm
 ...
 * 
	...
	4. Text field will blank
	5. Checkbox will unchecked and button 'Unblock Kartu ATM' will disabled
		...
		if (WebUI.verifyElementVisible(btnUnblockCard,FailureHandling.OPTIONAL)) {
			...
			if (WebUI.verifyElementPresent(unblockAtmCard, 5,FailureHandling.OPTIONAL)) {
				...
				if (WebUI.verifyElementClickable(btnUnblockAtmCard,FailureHandling.OPTIONAL)) {
					WebUI.verifyElementChecked(chkMotherName, 5)
					WebUI.click(chkMotherName)
					WebUI.verifyElementChecked(chkAccNumber, 5)
					WebUI.click(chkAccNumber)
					WebUI.verifyElementChecked(chkAskPhoneNumber, 5)
					WebUI.click(chkAskPhoneNumber)
					keylogger.markPassed('Uncheck things to confirm')
					WebUI.clearText(fieldReasonUnblockCard)
					keylogger.markPassed('Text field will blank')
					WebUI.takeScreenshot()
				} else {keylogger.logInfo("Element Not Found")}
				if (WebUI.verifyElementNotClickable(btnUnblockAtmCard,FailureHandling.OPTIONAL)) {
					keylogger.markPassed('Checkbox will unchecked and button Unblock Kartu ATM will disabled')
					WebUI.click(btnCancelUnblockCard)
				} else {keylogger.logInfo("Button is click able")}
				...
				if (WebUI.verifyElementPresent(noticeBlockCardDissapear, 5,FailureHandling.OPTIONAL)) {
					keylogger.markPassed('Status card Blokir Kartu ATM still shown')
					...
					keylogger.markPassed('Status card still Sudah Aktivasi')
					/* We want back to default for this request Id*/								
