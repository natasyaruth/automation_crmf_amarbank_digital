    ...
    if (WebUI.waitForElementVisible(SectionChangelog, 5)) {
        ...
        if (changeLogActivity == true) {
            ...
            if (WebUI.waitForElementVisible(DrpFilterActions, 5, FailureHandling.OPTIONAL)) {
						WebUI.selectOptionByIndex(DrpFilterSource, 'CSR Management')
						WebUI.verifyElementText(DrpFilterSource, 'CSR Management')
					} else {keyLogger.logInfo("We Not find the filter 'CSR Management'")}
				...
				WebUI.verifyMatch(SourcesCSRManagement, ChangelogSources, false)
