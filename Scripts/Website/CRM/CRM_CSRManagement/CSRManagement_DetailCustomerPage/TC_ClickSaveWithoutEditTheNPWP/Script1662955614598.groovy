		...
		if(WebUI.waitForElementClickable(btnBack, 5)) {
			...
			if(actStatusCust.equals(onProcessStat) || actStatusCust.equals(completedStat)) {
				...
				if(WebUI.waitForElementClickable(btnEditPersonalData, 3)) {
					...
					/* Insert value NPWP customer by get the text customer NPWP from CSR detail */
					...
					if(actTaxNumber.length().equals(20)) {
						...
						/* Verify mesage success is appear, if it's not exists will move to else statement */
						if(WebUI.waitForElementPresent(msgSuccessPersonalData, 3)) {
							
							/* Take a screenshot */
							WebUI.takeScreenshot()
							
							/* Mark case as passed and print info message*/
							keyLogger.markPassed("Message error was not appear. Case Success")	
							...
							/* Verify if the mesage error NPWP is appear, if it's not exists will move to else statement */
							if(WebUI.waitForElementPresent(msgErrorTaxNumber, 2)) {
								
								/* Take a screenshot */
								WebUI.takeScreenshot()
								
								/* Declarate variable msgErrorNPWP as string an store value of msgErrorTaxNumber */
								String msgErrorNPWP = WebUI.getText(msgErrorTaxNumber)
								
								/* Mark case as failed and print info message*/
								keyLogger.markFailed("Error massage "+msgErrorNPWP+" is appear. Case Failed")
								...
								/* Mark case as failed and print info message*/
								keyLogger.markFailed("Message success is not appear. Case Failed")
