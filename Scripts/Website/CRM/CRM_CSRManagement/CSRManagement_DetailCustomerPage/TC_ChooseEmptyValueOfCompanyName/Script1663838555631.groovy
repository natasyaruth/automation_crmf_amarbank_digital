		...
		if(WebUI.waitForElementClickable(btnBack, 5)) {
			...
			if(actStatusCust.equals(onProcessStat) || actStatusCust.equals(completedStat)) {
				...
				if(WebUI.waitForElementClickable(btnEditEmploymentData, 3)) {
					...
					/* Verify if the field text company name is exists, if it's not exists will move to else statement */
					if(WebUI.waitForElementPresent(txtCompanyName, 1)) {
						
						/* Clear text in text field company name */
						WebUI.clearText(txtCompanyName)
						
					} else {
						
						/* Select value in dropdown work type to make field text company name appearing*/
					...
					if(WebUI.waitForElementPresent(msgErrorCompanyName, 2)) {
						
						/* Storing text of not found to variable actMsgError*/
						actMsgError = WebUI.getText(msgErrorCompanyName)
