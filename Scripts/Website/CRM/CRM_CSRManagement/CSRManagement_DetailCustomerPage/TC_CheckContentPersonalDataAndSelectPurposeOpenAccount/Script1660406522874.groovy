 ...
 * 8. We want to select account purpose
			...
			if (WebUI.waitForElementVisible(DataCustomer, 5, FailureHandling.OPTIONAL)) {
				if (WebUI.verifyElementClickable(btnEditDatCustomer,FailureHandling.OPTIONAL)) {
					...
					if (WebUI.verifyElementVisible(drpDownPurposeOpenAccount,FailureHandling.OPTIONAL)) {
						WebUI.selectOptionByIndex(drpDownPurposeOpenAccount, index)
					...
					if (WebUI.verifyElementVisible(btnCancelPersonalData,FailureHandling.OPTIONAL)) {
					  WebUI.takeScreenshot()
					  WebUI.click(btnCancelPersonalData)
