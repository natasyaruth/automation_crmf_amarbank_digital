...
while (flagDataReqId == false) {
	...
	if (wordingCsrMgt == true) {
		...
		for (int i = 0; i < listRows.size() ; i++) {
			println('No. of rows: ' + listRows.size() +' and list rows number is' +i)
			List<WebElement> listCols = listRows.get(i).findElements(By.tagName('td'))
				if (listCols.get(6).getText().equalsIgnoreCase('Detail')) {
					...
					if (headerCustDetail.equalsIgnoreCase('Detil Nasabah')) {
									...
									if (motherName.equalsIgnoreCase(wordingAskMotherName)) {
										...
										if (accountNumber.equalsIgnoreCase(wordingAccountNumber)) {
											...
											if (phoneNumber.equalsIgnoreCase(wordingPhoneNumber)) {
												...
												WebUI.takeScreenshot()
												String lostCard = WebUI.getText(txtLostCard)
												String stolenCard = WebUI.getText(txtStolenCard)
												String closeCard = WebUI.getText(txtCloseCard)
												if (lostCard.equalsIgnoreCase(wordingLostCard)) {
													WebUI.verifyElementClickable(lostCardRb)
													if (stolenCard.equalsIgnoreCase(wordingStolenCard)) {
														WebUI.verifyElementClickable(stolenCardRb)
														if (closeCard.equalsIgnoreCase(wordingCloseCard)) {
															WebUI.verifyElementClickable(closeCardRb)
															boolean disableClick = WebUI.verifyElementNotClickable(btnSubmitReqNewCard)
															if (disableClick == true) {
																WebUI.verifyElementClickable(btnBatalReqNewCard)
																WebUI.takeScreenshot()
																keyLogger.markPassed("We are success to check the reason new card card")
																WebUI.click(btnBatalReqNewCard)
																break LoopCsrManagement
															} else {keyLogger.markError("Failed to check the radio button")}
														} else {keyLogger.markError("Error choose reason")}
													} else {keyLogger.markError("Error choose reason")}
												} else {keyLogger.markError("Error choose reason")}
						...
						keyLogger.markError("We are not in customer detail")
					}
