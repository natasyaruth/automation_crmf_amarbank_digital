		...
		if(WebUI.waitForElementClickable(btnBack, 5)) {
			...
			if(actStatusCust.equals(onProcessStat) || actStatusCust.equals(completedStat)) {
				...
				if(WebUI.waitForElementClickable(btnEditEmploymentData, 3)) {
					...
					/* Verify if the dropdown company field is exists, if it's not exists will move to else statement */
					if(WebUI.waitForElementPresent(drpCompanyField, 1)) {
						
						/* Select empty value (at index 0) in dropdown company field */
						WebUI.selectOptionByIndex(drpCompanyField, 0)
						
					} else {
						
						/* Select value in dropdown work type to make dropdown company field is appear*/
					...
					if(WebUI.waitForElementPresent(msgErrorCompanyField, 2)) {
						...
						actMsgError = WebUI.getText(msgErrorCompanyField)
