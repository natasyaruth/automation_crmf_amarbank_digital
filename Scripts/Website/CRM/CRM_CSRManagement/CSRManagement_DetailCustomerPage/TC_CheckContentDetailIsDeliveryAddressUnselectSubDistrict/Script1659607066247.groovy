		...
		if (addressSentCard == true) {
			...
			if (WebUI.waitForElementClickable(btnEditAddressDelivery, 5, FailureHandling.OPTIONAL)) {
				...
				if (chooseCity == true) {
					...
					WebUI.selectOptionByIndex(drpCity, index)
				...
				if (chooseSubDistrict == true) {
					...
					WebUI.selectOptionByIndex(drpSubDistricts, "0")
				...
				WebUI.takeScreenshot()
				WebUI.click(btnSave)
				if (WebUI.waitForElementVisible(txtAlertSelectSubDistrict, 5, FailureHandling.OPTIONAL)) {
					WebUI.verifyElementPresent(txtAlertSelectSubDistrict, 5)
					WebUI.verifyElementText(txtAlertSelectSubDistrict, "Kecamatan wajib diisi")
