 ...
 * 	1. Click button 'Unblock Kartu'
	...
	5. Delete reason of unblock
	6. Click Batal
	7. Click button 'Unblock Kartu'
		...
		if (WebUI.verifyElementPresent(pageBlockCardInfo, 5, FailureHandling.OPTIONAL)) {
			...
			if (checkWrd.contentEquals('Unblock Kartu')) {
				...
				if (WebUI.verifyElementClickable(btnUnblockCard,FailureHandling.OPTIONAL)) {
					...
					WebUI.takeScreenshot()
					/* We want to check all things in unblock kartu*/					
					if (WebUI.verifyElementClickable(btnCancelUnblockCard,FailureHandling.OPTIONAL)) {
						...
						if (WebUI.verifyElementVisible(chkAccNumber,FailureHandling.OPTIONAL)) {
							keylogger.markPassed('Checklist all things to confirm')
							WebUI.click(chkAccNumber)
							WebUI.takeScreenshot()
						} else {keylogger.logInfo("Element Not Found")}
						...
						if (WebUI.verifyElementVisible(chkAskPhoneNumber,FailureHandling.OPTIONAL)) {
							keylogger.markPassed('Checklist all things to confirm')
							WebUI.click(chkAskPhoneNumber)
							WebUI.takeScreenshot()
						} else {keylogger.logInfo("Element Not Found")}				
						...
						/* We want to back to default with uncheck all the things and clear field reason field*/
						if (WebUI.verifyElementChecked(chkAccNumber, 5,FailureHandling.OPTIONAL)) {
							keylogger.markPassed('Success to uncheck the things')
							WebUI.click(chkAccNumber)
							WebUI.takeScreenshot()
						} else {keylogger.logInfo("Element Not checked")}
						if (WebUI.verifyElementChecked(chkMotherName, 5,FailureHandling.OPTIONAL)) {
							keylogger.markPassed('Success to uncheck the things')
							WebUI.click(chkMotherName)
							WebUI.takeScreenshot()
						} else {keylogger.logInfo("Element Not checked")}
						if (WebUI.verifyElementChecked(chkAskPhoneNumber, 5,FailureHandling.OPTIONAL)) {
							keylogger.markPassed('Success to uncheck the things')
							WebUI.click(chkAskPhoneNumber)
							WebUI.takeScreenshot()
						} else {keylogger.logInfo("Element Not checked")}
						wrdReason = WebUI.getText(fieldUnblockCardReason)
						if (WebUI.verifyElementText(fieldUnblockCardReason, wrdingUnblockCardReason,FailureHandling.OPTIONAL)) {
							WebUI.clearText(fieldUnblockCardReason)
							keylogger.markPassed('Success to clear text field')
							WebUI.takeScreenshot()
						} else {keylogger.logInfo("Wording not same like before")}
