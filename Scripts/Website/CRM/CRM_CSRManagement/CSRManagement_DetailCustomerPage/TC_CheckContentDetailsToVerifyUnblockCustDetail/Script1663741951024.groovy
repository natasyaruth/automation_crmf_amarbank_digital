 ...
 * Precondition:
 * 	1. Already Login
	2. Already create process block card
 * 
 * And have steps:
 * 	1. Click CSR Management
	...
	1. In section Data Kartu ATM, show detail as below:

		Green button 'Unblock Kartu' (default enable)
		Card status (icon green check if 'Sudah Aktivasi' and icon red alert if 'Belum Aktivasi')
		Display block status with icon alert red 'Block Kartu ATM'
	...
	if (WebUI.verifyElementPresent(csrDetaiPage, 5,FailureHandling.OPTIONAL)) {
		...
		if (WebUI.verifyElementPresent(pageBlockCardInfo, 5, FailureHandling.OPTIONAL)) {
			...
			if (checkWrd.contentEquals('Unblock Kartu')) {
				keylogger.markPassed('Green button Unblock Kartu (default enable)')
