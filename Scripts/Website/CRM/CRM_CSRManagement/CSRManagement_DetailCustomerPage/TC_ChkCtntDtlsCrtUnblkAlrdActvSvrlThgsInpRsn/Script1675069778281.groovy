 ...
 * 	1. Click button 'Unblock Kartu'
	2. Checklist several of things to confirm
 ...
 * 
	...
	3. Reason will inputed in the text field and Button 'Unblock Kartu ATM' will disable
	4. Button unlock card is disable
 ...
 * so I try to back to unblock card for this request Id*/		
		if (WebUI.verifyElementVisible(btnUnblockCard,FailureHandling.OPTIONAL)) {
			...
			if (WebUI.verifyElementPresent(unblockAtmCard, 5,FailureHandling.OPTIONAL)) {
				...
				WebUI.click(chkMotherName)
				WebUI.click(chkAccNumber)
				keylogger.markPassed('Checklist several of things to confirm')
				...
				if (WebUI.verifyElementNotClickable(btnUnblockAtmCard,FailureHandling.OPTIONAL)) {
					keylogger.markPassed('Button cannot click able')
					WebUI.takeScreenshot()
				...
				if (WebUI.verifyElementPresent(noticeBlockCardDissapear, 5,FailureHandling.OPTIONAL)) {
					...
					keylogger.markPassed('Reason will not display in customer detail')
					WebUI.takeScreenshot()
				} else {keylogger.logInfo("Element Not Found")}
			} else {keylogger.logInfo("Element Not Found")}
		} else {keylogger.logInfo("Element Not Found")}
		/* We want to get back the request Id is Unblock Card as default*/		
		if (WebUI.verifyElementVisible(btnUnblockCard,FailureHandling.OPTIONAL)) {
			WebUI.click(btnUnblockCard)
			WebUI.click(chkMotherName)
			WebUI.click(chkAccNumber)
