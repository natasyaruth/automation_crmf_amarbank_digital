 ...
 * 5. We want to check reference name
 * 6. We want to check mother name
 * 7. We want to input alphabetic reference phone number
			...
			if (WebUI.waitForElementVisible(DataCustomer, 5, FailureHandling.OPTIONAL)) {
				if (WebUI.verifyElementClickable(btnEditDatCustomer,FailureHandling.OPTIONAL)) {
					...
					if (WebUI.verifyElementVisible(txtRefPhoneNumber,FailureHandling.OPTIONAL)) {
						WebUI.setText(txtRefPhoneNumber, RandomStringUtils.randomAlphabetic(13))
					...
					if (WebUI.verifyElementVisible(btnSavePersonalData,FailureHandling.OPTIONAL)) {
					  ...
					  if (WebUI.verifyElementVisible(alertReferencePhoneNumberIsAlphabetic,FailureHandling.OPTIONAL)) {
						  WebUI.verifyElementText(alertReferencePhoneNumberIsAlphabetic, "Nomor kontak kerabat tidak sesuai format")
						  WebUI.takeScreenshot()
