...
for (int i = 0; i < customerType.size(); i++) {
	...
	WebUI.delay(5)
     /*We want to check and input field monthly income in employment data*
	*We want to Check and click button save*/
    ...
    if (WebUI.waitForElementVisible(elementEmploymentData, 5)) {
        ...
        if (dataPekerjaan == true) {
            ...
            if (WebUI.waitForElementVisible(TxtCompanyName, 5, FailureHandling.OPTIONAL)) {
                if (WebUI.verifyElementClickable(BtnEditEmploymentData, FailureHandling.OPTIONAL)) {
                    WebUI.click(BtnEditEmploymentData)
						oldData = WebUI.getAttribute(TxtCompanyName, 'value')
				WebUI.delay(1)
					if (WebUI.verifyElementVisible(TxtCompanyName, FailureHandling.OPTIONAL)) {
						...
						newData = WebUI.getAttribute(TxtCompanyName, 'value')
						} else {
                ...
                WebUI.verifyMatch(ChangelogField, lbCompanyName , false)
				...
				WebUI.verifyMatch(ChangelogoldData, oldDataLog.toUpperCase(), false)
				...
				WebUI.verifyMatch(ChangelognewData, newDataLog.toUpperCase(), false)
