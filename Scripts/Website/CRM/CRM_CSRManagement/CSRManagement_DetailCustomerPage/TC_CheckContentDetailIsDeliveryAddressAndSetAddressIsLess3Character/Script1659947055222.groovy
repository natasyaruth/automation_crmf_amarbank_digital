		...
		if (addressSentCard == true) {
			...
			if (WebUI.waitForElementClickable(btnEditAddressDelivery, 5, FailureHandling.OPTIONAL)) {
				...
				if (inputFullAddress == true) {
					...
					WebUI.setText(txtFullAddress, RandomStringUtils.randomAlphanumeric(2))
				...
				WebUI.takeScreenshot()
				WebUI.click(btnSave)
				if (WebUI.waitForElementVisible(txtAlertFullAddress, 5, FailureHandling.OPTIONAL)) {
					WebUI.verifyElementPresent(txtAlertFullAddress, 5)
					WebUI.verifyElementText(txtAlertFullAddress, "Pastikan panjang karakter antara 3 dan 150")
					keyLogger.logInfo("We capture the alert text for fill the 'Alamat Lengkap'")
					WebUI.takeScreenshot()
				} else {keyLogger.logInfo("We don't find the alert in 'Alamat Lengkap'")}
			} else {
				WebUI.takeScreenshot()
