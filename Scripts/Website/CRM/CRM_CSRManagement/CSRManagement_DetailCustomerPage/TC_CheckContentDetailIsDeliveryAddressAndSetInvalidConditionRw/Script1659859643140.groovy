		...
		if (addressSentCard == true) {
			...
			if (WebUI.waitForElementClickable(btnEditAddressDelivery, 5, FailureHandling.OPTIONAL)) {
				...
				if (inputRw == true) {
					...
					WebUI.setText(txtRw,"-_-")
				} else (inputRw == false) { keyLogger.logInfo("We don't find the element Rw") }
				WebUI.waitForElementVisible(btnSave, 5)
				WebUI.takeScreenshot()
				WebUI.click(btnSave)
				if (WebUI.waitForElementVisible(txtAlertInvalidInputRw, 5, FailureHandling.OPTIONAL)) {
					WebUI.verifyElementPresent(txtAlertInvalidInputRw, 5)
					WebUI.verifyElementText(txtAlertInvalidInputRw, "RW tidak sesuai format")
