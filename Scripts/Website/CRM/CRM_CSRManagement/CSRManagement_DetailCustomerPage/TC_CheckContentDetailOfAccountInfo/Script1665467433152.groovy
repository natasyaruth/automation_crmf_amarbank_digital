	...
	for(int i=0;i<listRows.size();i++) {
		...
		if(WebUI.waitForElementClickable(btnBack, 5)) {
			...
			if(actStatusCust.equals(onProcessStat) || actStatusCust.equals(completedStat)) {
				...
				if(WebUI.verifyElementNotClickable(txtAccountNumber)) {
					
					/* Verify images of KTP and Selfie should complete */
					if(WebUI.verifyElementNotClickable(txtCardNumber)){
						
						/* Verify images of KTP and Selfie should complete */
						if(WebUI.verifyElementNotClickable(txtCardName)){
							
							keyLogger.markPassed("Account number, Card number and Card name are exists and non-editable. Case SUCCESS")
					
						} else {
							
							WebUI.takeScreenshot()
							keyLogger.markFailed("Card name is not exists and editable. Case FAILED")
						...
						WebUI.takeScreenshot()
						keyLogger.markFailed("Card number is not exists and editable. Case FAILED")
					...
					WebUI.takeScreenshot()
					keyLogger.markFailed("Account number is not exists and editable. Case FAILED")
