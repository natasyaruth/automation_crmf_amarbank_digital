...
/*Expected Result
	- Agen can check all in bucketlist
...
if (WebUI.verifyElementPresent(txtDashboard, 5)) {
	...
	while (flag == false) {
		for (int i = 0;i < rowsBackOffice.size(); i ++) {
			...
			if (colsBackOffice.get(1).getText().equalsIgnoreCase(numberQueue) && colsBackOffice.get(12).getText().equalsIgnoreCase("On Process")) {
				break dataCheck
