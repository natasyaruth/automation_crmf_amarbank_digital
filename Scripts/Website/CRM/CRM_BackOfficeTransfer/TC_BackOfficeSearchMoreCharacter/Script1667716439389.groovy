...
/*Step to process
Agen input one or mpre character in textfield search

/*Expected Result
Agen can input on or more character in textfiel search
Data table will be showing records who contains searching character
...
if (WebUI.verifyElementPresent(txtDashboard, 5)) {
	...
	if (WebUI.verifyOptionsPresent(drpDwnStatus, listDrpDwnStatus)) {
		WebUI.setText(fieldSearch, RandomStringUtils.randomAlphabetic(1))
		WebUI.setText(fieldSearch, RandomStringUtils.randomAlphabetic(30))
		WebUI.setText(fieldSearch, "On Process On Process On Process")
		...
		if (colsBackOffice.get(12).getText().equalsIgnoreCase("On Process")) {
			...
			println(seqNumb)
			colsBackOffice.get(0).findElement(By.xpath('input'))
	...
	WebUI.selectOptionByLabel(drpDwnShowRow, "10", false)
