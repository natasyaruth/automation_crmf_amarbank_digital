 ...
 * 	-User has access to KYC active video calls customer details
	-User go to other tabs or other page
	-User has access to CRMF dashboard page
	
	STEPS
	Click the pending request card

	EXPECTED RESULT
	System display previous KYC video active calls customer details
 */
		...
		if (WebUI.verifyElementClickable(btnCallSenyumku)) {
			...
			if (WebUI.verifyElementPresent(txtVerifConnect, 0)) {
				...
				if (WebUI.waitForElementVisible(pendingRequest, 5)) {
					...
					keylogger.markPassed('Pending Request is availaible')
