...
if (WebUI.waitForElementPresent(linkMenuCsrManagement, 5)) {
    ...
    keylogger.markError('We not found the CSR Management')
}
...
loopCheckData: while (checkData == false) {
            ...
            if (WebUI.waitForElementVisible(statusCust, 5)) {
                ...
                if (WebUI.waitForElementPresent(txtPhoneNumber, 5)) {
                    ...
                    if (WebUI.waitForElementVisible(btnSavePhoneNumber, 5)) {
                        ...
                        keylogger.markError('Element not visible')
                    }
                } else {
...
'We want to reject KYC Video'
WebUI.scrollToElement(BtnReject1, 5)

if (WebUI.verifyElementVisible(BtnReject1)) {
...
if (WebUI.verifyElementPresent(csrManagementDetailCheck, 5)) {
	WebUI.verifyTextPresent(TxtRejectionReason, false)
    WebUI.click(linkDataPhoneNumber)
} else {
...
TestObject SectionChangelogCheck = new TestObject().addProperty('text', ConditionType.CONTAINS, 'Tolak')
