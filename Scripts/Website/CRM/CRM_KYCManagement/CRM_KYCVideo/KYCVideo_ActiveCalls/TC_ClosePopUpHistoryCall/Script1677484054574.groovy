...
List<WebElement> colsKycVideo = rawKycVideo.get(3).findElements(By.tagName('td'))
...
if (WebUI.verifyElementVisible(btnKycFinished)) {
    ...
    if (WebUI.verifyElementPresent(historyCallCheck, 5)) {
		...
		WebUI.scrollToElement(BtnBackToKYC, 5)
        WebUI.delay(5)

        if (WebUI.verifyElementVisible(BtnBackToKYC)) {
            WebUI.click(BtnBackToKYC)

            WebUI.delay(10)


            if (WebUI.verifyElementPresent(BtnCancel, 5)) {
            ...
            keylogger.markError('Button back KYC video not click able')
        }
    } else {
