...
if (WebUI.waitForElementVisible(drpDwnEmailVerif, 5)) {
...
if (colsKycVerif.get(7).getText().equalsIgnoreCase("Menunggu")) {
	...
	if (WebUI.waitForElementVisible(kycDetailPageAfterFilter, 5)) {
		
		WebUI.scrollToElement(btnFaceMatch, 5)
		
		if (WebUI.waitForElementClickable(btnFaceMatch, 5)) {
			
			WebUI.click(btnFaceMatch)
			
		} else {
			
			keylogger.markFailed(' Button Face Match is Disable Please check another')
		}
		...
		WebUI.waitForElementVisible(kycBucketListAfterFilter, 5)
...
if (WebUI.waitForElementVisible(txtCsrDetail, 5)) {
