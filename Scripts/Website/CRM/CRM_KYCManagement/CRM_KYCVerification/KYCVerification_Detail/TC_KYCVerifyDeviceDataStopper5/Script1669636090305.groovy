 ...
 * 
 * Steps
 * 	- Customer registration with device id same as registered account before
	- User click on request ID customer no.1
	- Verify section "Device Data"
	
	Expected Result
	 - Section "Device Data" show Stopper 5
 * 
 * */
...
if (WebUI.verifyElementPresent(kycVerifBucketList, 5)) {
	WebUI.setText(txtReqIdKycVerif, reqIdStopper5)
...
if (colsKycVerif.get(2).getText().equalsIgnoreCase("Registrasi Baru")) {
	...
	colsKycVerif.get(1).getText().equalsIgnoreCase(reqIdStopper5)
...
'We want to check in Card Management as stopper 5'
...
WebUI.setText(txtReqIdCsrBucketList, reqIdStopper5)
...
if (WebUI.verifyElementPresent(csrDetail, 5)) {
	...
	if (WebUI.waitForElementPresent(lbStopper, 5)) {
		...
		TestObject checkStopper = new TestObject().addProperty('text',ConditionType.CONTAINS,'Stopper 5')
		if (WebUI.verifyElementPresent(checkStopper, 5)) {
			keylogger.markPassed("Stopper 5 already shown")
