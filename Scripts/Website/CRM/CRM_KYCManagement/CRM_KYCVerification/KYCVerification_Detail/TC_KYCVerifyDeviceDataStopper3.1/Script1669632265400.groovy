 ...
 * 	- Customer registration with device id same as registered account before
	...
	Expected Result
	 - Section "Device Data" show Stopper 3.1
 * 
 * */
...
if (colsKycVerif.get(2).getText().equalsIgnoreCase("Registrasi Baru")) {
	...
	colsKycVerif.get(1).getText().equalsIgnoreCase(reqIdStopper3)
...
'We want to check in Card Management as stopper 3.1'
...
if (colsCsrBucket.get(1).getText().equalsIgnoreCase(newUserName)) {
...
if (WebUI.verifyElementPresent(csrDetail, 5)) {
	...
	if (WebUI.waitForElementPresent(lbStopper, 5)) {
		...
		TestObject checkStopper = new TestObject().addProperty('text',ConditionType.CONTAINS,'Stopper 3.1')
		if (WebUI.verifyElementPresent(checkStopper, 5)) {
			keylogger.markPassed("Stopper 3.1 already shown")
