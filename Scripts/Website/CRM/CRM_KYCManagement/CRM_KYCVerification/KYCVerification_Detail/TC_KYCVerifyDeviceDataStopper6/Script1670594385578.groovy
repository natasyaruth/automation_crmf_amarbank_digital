 ...
 * 
 * Steps
 * 	- Customer registration with device emulator
	- User click on request ID customer no.1
	- Verify section "Device Data"
	
	Expected Result
	 - Section "Device Data" show Stopper 6
 * 
 * */
...
if (WebUI.verifyElementPresent(kycVerifBucketList, 5)) {
	WebUI.setText(txtReqIdKycVerif, reqIdStopper6)
...
if (colsKycVerif.get(2).getText().equalsIgnoreCase("Registrasi Baru")) {
	...
	colsKycVerif.get(1).getText().equalsIgnoreCase(reqIdStopper6)
...
if (WebUI.verifyElementPresent(csrDetail, 5)) {
	...
	if (WebUI.waitForElementPresent(lbStopper, 5)) {
		...
		TestObject checkStopper = new TestObject().addProperty('text',ConditionType.CONTAINS,'Stopper 6')
		if (WebUI.verifyElementPresent(checkStopper, 5)) {
			keylogger.markPassed("Stopper 6 already shown")
