...
/*Scenario Test
 * Precondition:
	- Already request change phonenumber with customer origin is eFishery
	- Login to CRMF
	- Already finished the video call process in KYC Video
	
	Steps:
	- Open detail customer in KYC Verification with customer type is Nasabah Senyumku, request type Change phonenumber and customer origin is eFishery
	- Accept the request
	
	Expected Result:
	- Button 'Terima' 'Lanjutkan KYC' and 'Tolak' is enable
	- Display pop up KYC Verification successfully accepted with button 'Kembali'
	- Phonenumber is changes in CSR Management
 */
...
println(GlobalVariable.phoneNumberWithAreaCode)
...
if (WebUI.verifyElementPresent(csrManagementBucketList, 5, FailureHandling.OPTIONAL)) {
	WebUI.delay(3)
	WebUI.setText(txtRequestIdCsr, ReqIdEfishery)
