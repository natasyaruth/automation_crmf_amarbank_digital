...
int countSnackbarError,countNothingHappened, countFailedToSaved = 0
		...
		if(WebUI.waitForElementClickable(btnBack, 5)) {
			...
			if(WebUI.waitForElementNotPresent(txtValueFMDukcapil, 2) && WebUI.waitForElementNotPresent(txtValueFMVerihub, 2)) {
				...
				if(WebUI.waitForElementClickable(btnCheckFMDukcapil, 5)) {
					...
					if(WebUI.waitForElementClickable(btnCheckFMVerihub, 2)) {
								
						keyLogger.markPassed("Button Facematch Dukcapil and Facematch Verihub are enabled after got error 500 when hit Facematch Dukcapil. "+"/n"+
											 "Case SUCCESS")
					...
					WebUI.takeScreenshot()
					keyLogger.markFailed("Button Facematch Dukcapil still disabled after got error 500 when hit Facematch Dukcapil."+"/n"+
										 "Please check again the configuration of Wiremock regarding Facematch Dukcapil and then re-run this case."+"/n"+
										 "Facematch Dukcapil should mock into Internal Server Error (500).")
