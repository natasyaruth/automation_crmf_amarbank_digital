...
if (onDetailAssignCard == true) {
	...
	if (WebUI.verifyElementVisible(btnEditDeliveryCard, FailureHandling.OPTIONAL)) {
		...
		if (WebUI.verifyElementVisible(fieldFullAddress,FailureHandling.OPTIONAL)) {
			boolean indexPresent =WebUI.verifyElementPresent(fieldFullAddress, 5)
			if (indexPresent == true) {
				WebUI.waitForElementVisible(fieldFullAddress, 5)
				WebUI.setText(fieldFullAddress, "Test" +RandomStringUtils.randomAlphanumeric(200))
