...
if (onDetailAssignCard == true) {
	...
	if (WebUI.verifyElementVisible(btnEditDeliveryCard, FailureHandling.OPTIONAL)) {
		...
		if (WebUI.verifyElementVisible(fieldRt,FailureHandling.OPTIONAL)) {
			boolean indexPresent =WebUI.verifyElementPresent(fieldRt, 5)
			if (indexPresent == true) {
				WebUI.waitForElementVisible(fieldRt, 5)
