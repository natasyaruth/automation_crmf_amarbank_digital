...
if (onDetailAssignCard == true) {
	...
	if (WebUI.verifyElementVisible(btnEditDeliveryCard, FailureHandling.OPTIONAL)) {
		...
		if (WebUI.verifyElementVisible(fieldRw,FailureHandling.OPTIONAL)) {
			boolean indexPresent =WebUI.verifyElementPresent(fieldRw, 5)
			if (indexPresent == true) {
				WebUI.waitForElementVisible(fieldRw, 5)
